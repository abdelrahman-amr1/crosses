...
async function testConnection() {
  for (const region of regions) {
    for (const port of ports) {
      ...
      console.log(`🔌 Testing: ${region} on ${host}:${port}...`);
      ...
      try {
        ...
        console.log(`🎯 TARGET FOUND! Connected to region: ${region} on ${host}:${port}`);
        ...
        if (!errMsg.includes("Tenant or user not found") && !errMsg.includes("postgres.vidahzporaivvfurnesx not found") && !errMsg.includes("ENOTFOUND")) {
          console.log(`⭐ promising response from ${host}:${port} - ${errMsg}`);
        } else {
          console.log(`❌ ${host}:${port} - ${errMsg}`);
        }
      }
    }
  }
  console.log("😢 Could not connect to any Format B pooler region/port.");
  return null;
}
